...
#     bridge_server_node.latest_prediction = {
#         'terrain_class': data['class'],
...
# # New Endpoint: Return the latest terrain prediction data
# @app.route('/predicted_data', methods=['GET'])
# def predicted_data():
#     global bridge_server_node
#     if bridge_server_node is None:
#         return jsonify({'error': 'BridgeServer node not available'}), 500
#     if bridge_server_node.latest_prediction is None:
...
#!/usr/bin/env python3
"""
ROS2 Dummy Server Node
Publishes dummy data for all expected REST endpoints used by the Drone Command Center frontend.
"""
import threading
import json
import rclpy
...
class DummyBridgeServer(Node):
    def __init__(self):
        super().__init__('dummy_bridge_server')
    ...
    def _register_routes(self):
        ...
        def current_position():
            return jsonify({'x': 1.23, 'y': 4.56, 'z': 7.89})

        @self.app.route('/target_position', methods=['GET'])
        def target_position():
            return jsonify({'x': 9.87, 'y': 6.54, 'z': 3.21})

        @self.app.route('/mode', methods=['GET', 'POST'])
        def mode():
            if request.method == 'GET':
                return jsonify({'mode': 'automatic'})
            data = request.get_json()
            mode = data.get('mode', 'automatic')
            return jsonify({'mode': mode, 'status': 'published'})

        @self.app.route('/launch', methods=['GET'])
        def launch():
            return jsonify({'status': 'launch command published'})

        @self.app.route('/drone_status', methods=['GET'])
        def drone_status():
            return jsonify({'phase': 2, 'launch_land_status': 'launched'})

        @self.app.route('/battery_status', methods=['GET'])
        def battery_status():
            return jsonify({'percentage': 85.5, 'voltage': 11.1})

        @self.app.route('/system_ready', methods=['GET'])
        def system_ready():
            return jsonify({'system_ready': True})

        @self.app.route('/velocities', methods=['GET'])
        def velocities():
            return jsonify({'flow_rate': {'x': 0.12, 'y': 0.34, 'z': 0.56}})

        @self.app.route('/all', methods=['GET'])
        def all_data():
            return jsonify({
                'current_position': {'x': 1.23, 'y': 4.56, 'z': 7.89},
                'target_position': {'x': 9.87, 'y': 6.54, 'z': 3.21},
                'battery_status': {'percentage': 85.5, 'voltage': 11.1},
                'system_ready': True,
                'velocities': {'flow_rate': {'x': 0.12, 'y': 0.34, 'z': 0.56}},
                'depth_cam_rgb': {'image': ''},
                'perpendicular_cam_rgb': {'image': ''},
                'drone_status': {'phase': 2, 'launch_land_status': 'launched'},
                'predicted_data': {'terrain_class': 'flat', 'accuracy': 0.95, 'coordinate': 'x=1.0 y=2.0 z=3.0', 'response_time': 0.123},
                'coordinates': {
                    'current': {'x': 1.23, 'y': 4.56, 'z': 7.89},
                    'origin': {'x': 0.0, 'y': 0.0, 'z': 0.0},
                    'orientation': {'ox': 0.0, 'oy': 0.0, 'oz': 0.0, 'ow': 1.0}
                }
            })

        @self.app.route('/reset_odom_services', methods=['GET'])
        def reset_odom():
            return jsonify({'status': 'success'})

        @self.app.route('/land_coordinates', methods=['GET'])
        def land_coordinates():
            return jsonify({'landing_coordinates': 'x=5.0 y=5.0 z=5.0'})

    def run(self):
        # Start Flask server in a separate thread
        threading.Thread(target=lambda: self.app.run(host='0.0.0.0', port=5000, use_reloader=False), daemon=True).start()
        self.get_logger().info('DummyBridgeServer Flask API running on port 5000')
        # Spin ROS node
        rclpy.spin(self)
