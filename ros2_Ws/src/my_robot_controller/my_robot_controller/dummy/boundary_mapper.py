...
class LineFollowerNode(Node):
    ...
    def phase_callback(self, msg: String):
        ...
        self.get_logger().info("Phase updated: %s", self.current_phase)

    def get_image_from_server(self):
        """
        Sends a GET request to http://localhost:5000/latest_image
        and returns the decoded OpenCV image if successful.
        """
        try:
            response = requests.get("http://localhost:5000/latest_image", timeout=1.0)
            if response.status_code == 200:
                ...
                if image_b64:
                    ...
                    return img
            self.get_logger().error("Failed to get image from server (status: %s)", response.status_code)
        except Exception as e:
            self.get_logger().error("Error getting image: %s", str(e))
        return None
    ...
    def timer_callback(self):
        ...
        self.get_logger().info("[STATUS] %s", status_msg)
        self.get_logger().info("[COMMAND] %s", command_msg)
        self.get_logger().info("[DETAIL] %s", details)
