...
def generate_launch_description():
    ...
    # 1) MAVROS launch in its own terminal
    ld.add_action(
        ExecuteProcess(
    ...
    # 2) after 30s, set stream rate 10 in new terminal
    ld.add_action(
        TimerAction(
            period=30.0,
            actions=[
    ...
    # 4) after 44s, launch realsense in new terminal
    ld.add_action(
        TimerAction(
            period=44.0,
            actions=[
    ...
    # 5) after 59s, launch static transforms
    ld.add_action(
        TimerAction(
            period=59.0,
            actions=[
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='ros2 launch static_transforms_pkg static_transforms.launch.py; exec bash',
                    output='screen'
                )
            ]
        )
    ...
    # 6) after 69s: clear db and launch rtabmap in own terminal
    ld.add_action(
        TimerAction(
            period=69.0,
            actions=[
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='rm -f ~/.ros/rtabmap.db; exec bash',
                    output='screen'
                ),
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
    ...
    ld.add_action(
        TimerAction(
            ...
            actions=[
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='ros2 launch ekf_imu_fusion_pkg ekf_imu_fusion.launch.py; exec bash',
                    output='screen'
                )
            ]
        )
    ...
    ld.add_action(
        TimerAction(
            ...
            actions=[
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='ros2 run v4l2_camera v4l2_camera_node; exec bash',
                    output='screen'
                )
            ]
        )
    ...
    # 9–11) guided, arm, takeoff after 75s
    ld.add_action(
        TimerAction(
            period=75.0,
            actions=[
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='ros2 service call /mavros/set_mode mavros_msgs/srv/SetMode "{base_mode:0, custom_mode: \'GUIDED\'}"; exec bash',
                    output='screen'
                ),
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='ros2 service call /mavros/cmd/arming mavros_msgs/srv/CommandBool "{value: true}"; exec bash',
                    output='screen'
                ),
                ExecuteProcess(
                    prefix=['gnome-terminal', '--', 'bash', '-c'],
                    cmd_str='ros2 service call /mavros/cmd/takeoff mavros_msgs/srv/CommandTOL "{min_pitch:0.0, yaw:0.0, latitude:0.0, longitude:0.0, altitude:1.8}"; exec bash',
                    output='screen'
                ),
            ]
        )
